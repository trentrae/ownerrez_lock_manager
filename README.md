...
- Three binary sensors for same-day check-in, guest arrival, and lock readiness
...
2. Open it and install the latest release (**v2.0.5**)
...
> Already on v2.x? Updating to **v2.0.5** is in-place through HACS and does not require re-adding the integration.
  ...
  - type: entities
    ...
    entities:
      - entity: sensor.ownerrez_booking_status
        name: Status
        icon: mdi:home-clock
      - entity: binary_sensor.ownerrez_guest_arrived
        name: Guest Arrived
...
All configured lock entities are watched for state changes to `unlocked` while a guest code is active:
...
- Removed optimistic slot-state success marking so `code_active` no longer appears healthy unless programming verification actually succeeds on every lock

### v2.0.11
- Improved lock programming health fallback so idle locks without slot telemetry show as cleared instead of unknown when no guest code is active
...
- Improved idle-state display for Z-Wave locks so health does not remain unknown when no guest code is active

### v2.0.8
- Fixed `sensor.ownerrez_booking_status` to prioritize active-stay time window, preventing incorrect `booking_pending` while a guest is currently in stay
- Fixed `binary_sensor.ownerrez_same_day_checkin` to suppress same-day alert during an active stay and use next booking check-in datetime when available

### v2.0.7
- Added `binary_sensor.ownerrez_guest_arrived` so the dashboard can show a direct arrived/not-arrived state
- Updated the current-stay dashboard example to include the guest-arrived indicator

### v2.0.6
- Updated arrival detection so any configured lock can mark the guest as arrived on first valid unlock use
- Kept the Z-Wave notification path active so lock user/slot data can still be attributed when available

### v2.0.5
- Added a current time/date guard so the coordinator does not switch to or program a future guest before the current guest's checkout time
- Updated manual clear behavior during an active stay to preserve current booking context instead of advancing to the next booking
